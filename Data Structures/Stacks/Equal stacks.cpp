...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    ...
    i--;
    for (; i >= 0; i--)
        s1.push(h1[i]+s1.top()); // stack s1 = [1, 2, 3, 5, 8>
    
    i = h2.size()-1;
    s2.push(h2[i]);
    i--;
    for (; i >= 0; i--)
        s2.push(h2[i]+s2.top()); // stack s2 = [2, 5, 9>
    
    i = h3.size()-1;
    s3.push(h3[i]);
    i--;
    for (; i >= 0; i--)
        s3.push(h3[i]+s3.top()); // stack s3 = [1, 5, 6, 7>

    while (!s1.empty() && !s2.empty() && !s3.empty()) { // if any stack gets empty then the answer is 0

        if (s1.top() == s2.top() && s1.top() == s3.top()) // if top of all stacks is equal
            return s1.top();

        if (s1.top() >= s2.top() && s1.top() >= s3.top()) // if top of s1 is largest
            s1.pop();
        else if (s2.top() >= s1.top() && s2.top() >= s3.top()) // if top of s2 is largest
            s2.pop();
        else // if top of s3 is largest
            s3.pop();
    }
    return 0;
}
